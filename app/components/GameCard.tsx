...
export function GameCard({ jogo, onEditClick, onDeleteClick }: GameCardProps) {
  ...
  return (
    ...
    className="relative bg-slate-800/60 backdrop-blur-sm rounded-xl border border-slate-700/50 shadow-lg p-6"
  >
    <button
      onClick={onEditClick}
      aria-label="Editar jogo"
      className="absolute top-3 left-3 p-2 rounded-full text-slate-400 hover:bg-sky-500/20 hover:text-sky-400 transition-colors z-10 cursor-pointer"
    >
    ...
    <button
      ...
      className="absolute top-3 right-3 p-2 rounded-full text-slate-400 hover:bg-red-500/20 hover:text-red-400 transition-colors z-10 cursor-pointer"
    >
      <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 6h18" />
